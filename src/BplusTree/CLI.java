...
import FileManager.RecordManager;
import BplusTree.*;
...
public class CLI {
	...
	private static void clearAndDelete() {
		if(tree != null){
			log("Deleted index file: "+tree.getFileManagerBase().deleteFile());
			tree = null;
		}else{
	...
	public static void queryTree(){
		...
		tree.getFileManagerBase().resetReadWriteCounter();
		log("Key to query:");
		int key = Integer.parseInt(sc.nextLine());
		...
		log("Amount of disk accesses: " + tree.getFileManagerBase().getNumberOfReadWrites());
